...
const PlatoCard = ({ plato } : PlatoCardProps) => {
  return (
    <div className='bg-white rounded-2xl shadow-lg hover:2xl transform-all duration-300 overflow-hidden
    border border-gray-100 group'>
      <div className='relative h-48 overflow-hidden'>
        ...
        alt={plato.nombre} />
        <div className='absolute top-4 right-4 bg-amber-100 px-3 rounded-full bg-opacity-50 text-xs font-bold text-orange-600
        shadow-sm'>
          {plato.categoria}
        </div>
      </div>
        ...
        <div>
          ...
          <div className='flex flex-row flex-wrap gap-1'>
            {plato.ingredientes.map((ingrediente : string, index : number) => (
              <span 
              key={index}
              className='text-[10px] px-2 py-1 bg-orange-50 text-orange-600 rounded-md border-orange-100 m-1'
              >
                {ingrediente}
              </span>
            ))}
